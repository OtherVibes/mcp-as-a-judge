...
class RepositoryAnalyzer:
    ...
    def _detect_languages(self) -> Dict:
        ...
        return {
            ...
            "confidence": "high" if language_scores and max(language_scores.values()) >= 10 else "medium" if language_scores else "low"
        }
    ...
    def _detect_frameworks(self) -> Dict:
        ...
        return {
            ...
            "possible": [fw for fw, score in framework_scores.items() if 1 <= score < 5]
        }
    ...
    def _parse_pom_xml(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            if '<dependencies>' in content:
                deps_section = content.split('<dependencies>')[1].split('</dependencies>')[0]
                # Extract artifactId values (simplified)
                import re
                artifact_ids = re.findall(r'<artifactId>(.*?)</artifactId>', deps_section)
    ...
    def _generate_recommendations(self) -> Dict:
        ...
        if languages["confidence"] == "high" and languages["primary"]:
            ...
            recommendations["reasoning"].append(f"Clear {languages['primary']} project detected - recommend continuing with {languages['primary']} for consistency")
            recommendations["reasoning"].append("However, user may choose different language for valid reasons (microservices, tooling, team expertise, etc.)")

            if frameworks["likely"]:
                recommendations["reasoning"].append(f"Likely frameworks: {', '.join(frameworks['likely'])}")
            ...
            recommendations["reasoning"].append("Multiple languages detected or unclear primary language")
        else:
    ...
    def get_summary_text(self) -> str:
        ...
        if languages["primary"]:
            summary_parts.append(f"**Primary Language**: {languages['primary'].title()}")
        ...
        if frameworks["likely"]:
            summary_parts.append(f"**Likely Frameworks**: {', '.join(frameworks['likely'])}")
        elif frameworks["possible"]:
            summary_parts.append(f"**Possible Frameworks**: {', '.join(frameworks['possible'])}")
        
        # Structure summary
        structure = analysis["project_structure"]
        if structure["patterns"]:
            summary_parts.append(f"**Project Patterns**: {', '.join(structure['patterns'])}")
        
        # Recommendations
        recommendations = analysis["recommendations"]
        if recommendations["suggested_approach"] == "confirm_or_choose_different":
            summary_parts.append(f"**Recommendation**: Continue with {languages['primary']} for consistency, but user choice is respected")
            summary_parts.append("**Note**: User may choose different language for valid architectural reasons")
        else:
            summary_parts.append(f"**Recommendation**: {recommendations['suggested_approach']}")

        if recommendations["reasoning"]:
            summary_parts.append(f"**Analysis**: {'; '.join(recommendations['reasoning'])}")
