...
class PlanFormatter:
    ...
    def _create_executive_summary(
        ...
        if technical_decisions:
            ...
            tech_summary = ", ".join(
                [
                    f"{d.get('decision', 'Unknown')}: {d.get('choice', 'Unknown')}"
                    for d in key_tech
                ]
            )
    ...
    def generate_user_questions(
        ...
        if technical_decisions:
            questions.append(
                "Do the technical choices align with your project goals and constraints?"
            )
            ...
            for decision in technical_decisions:
                ...
                if "database" in decision_type:
                    questions.append(
                        f"Are you comfortable with {choice} for data storage and scalability needs?"
                    )
                elif "framework" in decision_type or "library" in decision_type:
                    questions.append(
                        f"Does {choice} fit your team's expertise and project timeline?"
                    )
                elif "api" in decision_type:
                    questions.append(
                        f"Will {choice} meet your integration and client requirements?"
                    )
        ...
        if "high" in complexity or "complex" in complexity:
            questions.append(
                "Given the complexity, would you prefer to break this into smaller phases?"
            )

        total_files = len(implementation_scope.get("files_to_create", [])) + len(
            implementation_scope.get("files_to_modify", [])
        )
        if total_files > 10:
            questions.append(
                "This involves many files - are there any specific areas you'd like to prioritize?"
            )

        # Questions about implementation approach
        if "test" in plan_content.lower():
            questions.append(
                "What level of test coverage do you expect for this implementation?"
            )

        if "deploy" in plan_content.lower() or "production" in plan_content.lower():
            questions.append(
                "Do you have specific deployment or production environment requirements?"
            )

        # Always ask about timeline and priorities
        questions.append(
            "Are there any specific deadlines or priorities that should influence the implementation order?"
        )
        questions.append(
            "Would you like any additional features or considerations included in this plan?"
        )
