...
def test_judge_coding_plan_signature():
    ...
    assert sig.parameters['plan'].default == inspect.Parameter.empty, "plan should be required"
    assert sig.parameters['design'].default == inspect.Parameter.empty, "design should be required"
    assert sig.parameters['research'].default == inspect.Parameter.empty, "research should be required"
    ...
    assert return_annotation.__name__ == 'JudgeResponse', f"Expected JudgeResponse return type, got {return_annotation}"
...
def test_function_docstring():
    ...
    assert 'MANDATORY VALIDATION' in docstring, "Should have mandatory usage description"
